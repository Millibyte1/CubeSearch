...
import io.github.millibyte1.cubesearch.util.PatternDatabaseUtils
...
class CornerPatternDatabase private constructor(
    ...
    private val cardinality = POWERS_OF_THREE[consideredCorners.size].toLong() * (FACTORIALS[8] / FACTORIALS[8 - consideredCorners.size])

    private val factory = SmartCubeFactory()
    private var table = FastByteArray64(cardinality) { -1 }

    init {
        val bytes = core.readDatabase64()
        ...
        if(bytes == null) {
            ...
            when(searchMode) {
                ...
                "dfs" -> PatternDatabaseUtils.populateDatabaseDFS(maxCost, table, this)
            }
            //stores the database in the core for persistent storage
            core.writeDatabase64(table)
        }
    ...
    override fun getIndex(cube: AnalyzableStandardCube): Long {
     ...
     */
    internal fun getPositionIndex(cube: AnalyzableStandardCube): Long {
    ...
    internal fun getOrientationIndex(cube: AnalyzableStandardCube): Long {
    ...
    internal override fun getPopulation(): Long {
        return table.fold(0) { total, item -> if(item == (-1).toByte()) total else total + 1 }
    }

    override fun getCardinality(): Long {
        return cardinality
    }
