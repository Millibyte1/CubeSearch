...
import io.github.millibyte1.cubesearch.algorithm.heuristics.*
import io.github.millibyte1.cubesearch.cube.*
import io.github.millibyte1.cubesearch.util.CubeGenerator
...
class SolverTest {
    ...
    */
    ...
    fun testRandomSolutions(solver: Solver) {
        val timesByDepth = Array<ArrayList<Long>>(depthRating(solver) + 1) { ArrayList() }
        for(depth in 0..depthRating(solver)) {
            generator.reset()
            generator.setWalkLength(depth)
            for(i in 0 until 100) {
                //solves the cube and measures the time it took to solve it
                var cube = generator.nextCube()
                var solution: List<Twist>?
                val time = measureTimeMillis { solution = solver.getSolution(cube) }
                ...
                //inserts the time it took to solve this cube into the timings
                timesByDepth[solution!!.size].add(time)
            }
            println("Finished solving cubes with random walk length $depth")
        }
        //prints timings
        for(depth in timesByDepth.indices) println("# of cubes with optimal solution of depth $depth: ${timesByDepth[depth].size}")
        for(depth in timesByDepth.indices) println("Average time to solve cube with depth $depth solution: ${timesByDepth[depth].average()}")
    }
    ...
    fun testSingleDeepSolution(solver: Solver) {
        ...
        println("Actual solution depth: ${solution!!.size}")
    ...
    companion object {
        ...
        private fun depthRating(solver: Solver): Int {
            return when(solver) {
                is io.github.millibyte1.cubesearch.algorithm.ClassicalAStarSolver -> 8
                is FrontierSearchSolver -> 8
                is IterativeDeepeningAStarSolver -> 13
                else -> 8
            }
        }
        @JvmStatic
